...
import { LogOut, MapPin, Menu, Package, Receipt, UserCircle2, X } from "lucide-react";
import { logoutAction } from "@/app/actions/auth";

const nav = [
  { href: "/vendor/tienda", label: "Mi Tienda (Ubicacion)", icon: MapPin },
  { href: "/vendor/productos", label: "Mis Productos", icon: Package },
  { href: "/vendor/pedidos", label: "Mis Pedidos", icon: Receipt }
];
...
export function VendorShell({ children, profileName, profileRole }: VendorShellProps) {
  const pathname = usePathname();
  const [mobileOpen, setMobileOpen] = useState(false);
  const displayName = profileName?.trim() || "Distribuidor";
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      >
        <div className="flex h-16 items-center justify-between border-b border-slate-100 px-5">
          <Link href="/vendor/productos" className="flex items-center gap-2 font-semibold tracking-tight text-slate-900">
